...
swipesByLine = map addLine . groupBy ((==) `on` (fst . swipeStart)) . concatMap splitSwipe
  where
    splitSwipe s@(Swipe (fromLine, _) (toLine, _) _)
      | fromLine == toLine = [s]
      | otherwise =
        s {swipeEnd = (fromLine, maxBound)} :
...
files ci@(CoverageIndex metadataMap) (CoverageReport annots) = sequence [file n | n <- fileNames ci]
  where
    file name = do
      body <-
        either (const "" :: IOException -> String) id
          <$> try (readFile name)
...
locSwipe loc status =
  Swipe
    (_covLocStartLine loc, _covLocStartCol loc)
  ...
  where
    ...
    header =
      element "h1" [] "Files"
        ++ element
          ...
          []
          ( concat
              [ element "li" [] . element "a" [("href", quote ("#" ++ name))] $ name
                | (name, _, _, _) <- fs
              ]
          )
    file name body covx annots =
      let uncovered = covx Set.\\ annots
          swipes =
            [ Swipe
                ...
                $ case ( CoverBool loc True `Set.member` uncovered,
                         CoverBool loc False `Set.member` uncovered
                       ) of
                  (True, True) -> Uncovered
       ...
       in element "h2" [("id", quote name)] name
            ++ element
              "pre"
              []
              ( unlines
...
baseCoverage covs =
  ...
  where
    outermost = reverse . removeNested . reverse
...
swipeLine c line s@(Swipe (_, from) (_, to) stat : swipes)
  | c < from =
    element "span" [("style", statusStyle OffChain)] (encode $ take (from - c) line)
      ++ swipeLine from (drop (from - c) line) s
  | otherwise =
    element "span" [("style", statusStyle stat)] (encode $ take (to + 1 - from) line)
