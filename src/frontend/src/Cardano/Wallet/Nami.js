...
exports.getWalletIdImpl = () => {
  return Promise.all([
    ...
    window.cardano.getUnusedAddresses(),
...
exports.signDataImpl = (address) => (payload) => () =>
...
exports.signTxImpl = (tx) => (partialSign) => () =>
  window.cardano.signTx(tx, partialSign);
...
const fetchProtocolParameters = () => {
  return fetch("https://cardano-testnet.blockfrost.io/api/v0/blocks/latest", {
    ...
    .then((latestBlock) => {
      return fetch(
        `https://cardano-testnet.blockfrost.io/api/v0/epochs/${latestBlock.epoch}/parameters`,
        {
          headers: {
            project_id: "testnetav02u9IsCvAzz50tCzKIhDpmtxEWOpuv",
          },
